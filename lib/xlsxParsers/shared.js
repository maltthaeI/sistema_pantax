...
// Coluna "Categoria" da planilha — usada só em Recebidas pra separar Revenda
// x Uso e Consumo (Emitidas/CT-e continuam usando cfop_direcao). Normaliza pra
// não depender de acentuação/capitalização exatas da planilha.
export function normalizarCategoria(categoria) {
    const c = String(categoria || '').trim().toLowerCase();
    if (c.includes('revenda')) return 'revenda';
    if (c.includes('uso') || c.includes('consumo')) return 'uso_consumo';
    return null;
}
