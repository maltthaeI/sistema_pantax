...
const CAMPOS_SOMADOS = ['valor_total', 'valor_ipi', 'valor_icms', 'valor_icms_st', 'valor_icms_uf_destino', 'valor_icms_simples_nacional'];
...
function agregarPorCfop(registros) {
    ...
    for (const r of registros) {
        ...
        // categoria entra na chave pra não misturar Revenda x Uso e Consumo do mesmo CFOP (Recebidas).
        const chave = `${r.cfop_direcao}:${r.cfop}:${r.categoria || ''}`;
        if (!grupos.has(chave)) {
            grupos.set(chave, { cfop: r.cfop, cfop_direcao: r.cfop_direcao, categoria: r.categoria || null, quantidade: 0, valor_total: 0, valor_ipi: 0, valor_icms: 0, valor_icms_st: 0, valor_icms_uf_destino: 0, valor_icms_simples_nacional: 0 });
        }
...
// Arquivo de CT-e sempre vem com mais de uma aba (ex: um resumo em tabela
// dinâmica + a aba de dados de verdade) e o nome da aba de dados varia entre
// exportações — por isso, ao invés de exigir um nome fixo, procura a primeira
// aba que tenha coluna "CFOP" (a de resumo/pivot não tem).
function encontrarAbaComCfop(workbook) {
    for (const nome of workbook.SheetNames) {
        const linhas = XLSX.utils.sheet_to_json(workbook.Sheets[nome], { defval: null });
        if (linhas.length > 0 && Object.prototype.hasOwnProperty.call(linhas[0], 'CFOP')) {
            return { nome, linhas };
        }
    }
    return null;
...
// Emitidas/Recebidas usam a aba fixa "Relatório Detalhado por Produto"; CT-e
// tem planilha própria (ver parseCte.js) com nome de aba variável.
function lerRegistrosDoArquivo(buffer, tipo) {
    const workbook = XLSX.read(buffer, { type: 'buffer' });
    if (tipo === 'cte') {
        const achada = encontrarAbaComCfop(workbook);
        if (!achada) {
            throw new Error(`Planilha de CT-e não tem nenhuma aba com coluna "CFOP". Abas encontradas: ${workbook.SheetNames.join(', ')}`);
        }
        return { linhas: achada.linhas, registros: parseCteRows(achada.linhas) };
    }
