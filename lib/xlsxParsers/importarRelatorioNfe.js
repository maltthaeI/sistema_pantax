...
const CAMPOS_SOMADOS = ['valor_total', 'valor_ipi', 'valor_icms', 'valor_icms_st', 'valor_icms_uf_destino', 'valor_pis', 'valor_cofins'];
...
function agregarPorCfop(registros) {
    ...
    for (const r of registros) {
        ...
        if (!grupos.has(chave)) {
            grupos.set(chave, { cfop: r.cfop, cfop_direcao: r.cfop_direcao, quantidade: 0, valor_total: 0, valor_ipi: 0, valor_icms: 0, valor_icms_st: 0, valor_icms_uf_destino: 0, valor_pis: 0, valor_cofins: 0 });
        }
...
function lerRegistrosDoArquivo(buffer, tipo) {
    ...
    if (!workbook.SheetNames.includes(ABA_RELATORIO)) {
        throw new Error(`Planilha de ${tipo} não tem a aba esperada ("${ABA_RELATORIO}"). Abas encontradas: ${workbook.SheetNames.join(', ')}`);
    }
...
// Recebe as 3 planilhas (Emitidas, Recebidas, CT-e) — mesmo formato de
// "Relatório Detalhado por Produto" — e agrega CADA UMA separadamente por
// CFOP (origem vira 3 telas independentes, não uma combinada).
