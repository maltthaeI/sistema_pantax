...
export async function POST(request) {
    ...
    const { data: batch } = await admin.from('import_batches').insert([{
        empresa_id: empresaId, tipo_arquivo: 'emitidas', nome_arquivo: file.name, status: 'processando', importado_por: usuario.id,
    }]).select().single();

    try {
        const buffer = await file.arrayBuffer();
        const resultado = await importarNotasFiscais(admin, { empresaId, direcao: 'emitida', buffer, batchId: batch.id });
