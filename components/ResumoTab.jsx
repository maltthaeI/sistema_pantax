...
export default function ResumoTab() {
    const { empresaAtualId, empresaAtual, competenciaAtual, setAbaAtual } = useAppContext();
    ...
    const toggleTodos = (linhasGrupo, marcar) => {
        setSelecionados(prev => {
            const novo = new Set(prev);
        ...
        <div className="p-6 max-w-6xl mx-auto flex flex-col gap-6">
            <div className="flex items-center gap-2">
                ...
                <h4 className="text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    Resumo — {empresaAtual?.nome_fantasia || empresaAtual?.razao_social} · {formatarMesAnoAbrev(competenciaAtual.ano, competenciaAtual.mes)} · {competenciaAtual.tipo_calculo === 'previa' ? 'Prévia' : 'Fechamento'}
                </h4>
            </div>
