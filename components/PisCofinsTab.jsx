...
function Bloco({ titulo, debito, credito, resultado }) {
    const aPagar = resultado > 0;
    return (
        <div className="flex flex-col gap-4">
            <h3 className="text-[12px] font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">{titulo}</h3>
...
export default function PisCofinsTab() {
    ...
    const { debitoPis, creditoPis, resultadoPis, debitoCofins, creditoCofins, resultadoCofins } = apuracaoPisCofins;
    ...
    return (
        <div className="p-6 max-w-3xl mx-auto flex flex-col gap-8">
            ...
            <Bloco titulo="PIS" debito={debitoPis} credito={creditoPis} resultado={resultadoPis} />
            <Bloco titulo="COFINS" debito={debitoCofins} credito={creditoCofins} resultado={resultadoCofins} />
        </div>
    );
}
