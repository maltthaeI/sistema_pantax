        ...
        <>
            {modalUsuarioAberto && (
                <div onClick={() => setModalUsuarioAberto(false)} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 dark:bg-black/80 glass no-print cursor-pointer animate-modal-backdrop">
                    <div onClick={e => e.stopPropagation()} className="bg-lightBg dark:bg-darkBg w-full max-w-sm rounded border border-gray-200 dark:border-darkBorder shadow-2xl flex flex-col max-h-[95vh] cursor-default overflow-hidden animate-modal-in">
                        ...
                        <form onSubmit={salvarUsuario} className="p-6 overflow-y-auto custom-scrollbar flex flex-col gap-4">
                            <div>
                                <label className="block text-[13px] font-medium mb-1.5 text-gray-700 dark:text-darkText">Nome</label>
                                <input required value={novoUsuario.nome} onChange={e => setNovoUsuario({ ...novoUsuario, nome: e.target.value })} className="w-full bg-white dark:bg-darkElevated border border-gray-300 dark:border-darkBorder rounded px-3 py-2 text-[13px] outline-none focus:border-brand transition dark:text-darkText" />
                            </div>
                            {!novoUsuario.id && (
                                <div>
                                    <label className="block text-[13px] font-medium mb-1.5 text-gray-700 dark:text-darkText">E-mail</label>
                                    <input required type="email" value={novoUsuario.email} onChange={e => setNovoUsuario({ ...novoUsuario, email: e.target.value })} className="w-full bg-white dark:bg-darkElevated border border-gray-300 dark:border-darkBorder rounded px-3 py-2 text-[13px] outline-none focus:border-brand transition dark:text-darkText" />
                                </div>
                            )}
                            <div>
                                <label className="block text-[13px] font-medium mb-1.5 text-gray-700 dark:text-darkText">Nível</label>
                                <CustomSelect
